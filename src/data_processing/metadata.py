...
# ¼ÓÔØ¾²Ì¬Êý¾Ý
country_code = pd.read_csv('Code/CountryCode.csv')
china_area_code = pd.read_csv("Code/ChinaAreaCode.csv")
...
def get_china_city_code(province_code, name):
    if not name or not province_code:
        return ""
    result = china_area_code.loc[
        china_area_code["province_code"].isin([province_code]) & ~china_area_code["is_province"] & china_area_code[
            "name"].str.contains(name)]["code"]
    if (len(result.values) > 0):
        return result.values[0]
    ...
    for i in range(1, len(name)):
        ...
        result = china_area_code.loc[
            china_area_code["province_code"].isin([province_code]) & ~china_area_code["is_province"] & china_area_code[
                "name"].str.match(fuzzy_name)]["code"]
        if (len(result.values) > 0):
